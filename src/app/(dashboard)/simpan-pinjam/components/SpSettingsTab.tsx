...
export default function SpSettingsTab() {
  const settings = useSettings() as any
  const [waNumber, setWaNumber] = useState("")
  
  // Page Content Settings
  const [descriptionPage, setDescriptionPage] = useState("")
  const [benefitsPage, setBenefitsPage] = useState("")
  ...
  useEffect(() => {
    if (settings) {
      setWaNumber(settings.simpan_pinjam_whatsapp || "")
      setDescriptionPage(settings.simpan_pinjam_description || "")
      setBenefitsPage(settings.simpan_pinjam_benefits || "")
      setRatesPage(settings.simpan_pinjam_rates || "")
      setRequirementsPage(settings.simpan_pinjam_requirements || "")
    }
  }, [settings])
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        simpan_pinjam_whatsapp: waNumber,
        simpan_pinjam_description: descriptionPage,
        simpan_pinjam_benefits: benefitsPage,
        simpan_pinjam_rates: ratesPage,
        simpan_pinjam_requirements: requirementsPage
      }
      ...
      if (result.success) {
        ...
        setMessage({ type: "success", text: "Pengaturan unit Simpan Pinjam berhasil disimpan!" })
        setTimeout(() => {
    ...
    <div className="space-y-6 max-w-xl animate-fade-in">
      ...
      <form onSubmit={handleSaveSettings} className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm space-y-4">
        ...
        <p className="text-slate-400 text-[11px] font-semibold leading-relaxed">
          Masukkan informasi kontak dan isi konten untuk halaman detail Simpan Pinjam publik.
        </p>
        ...
        <div className="space-y-3 pt-2">
          <div className="space-y-1">
            ...
            <input
              ...
              placeholder="Contoh: 6281234567892"
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Deskripsi Unit Usaha</label>
            <textarea
              value={descriptionPage}
              onChange={(e) => setDescriptionPage(e.target.value)}
              placeholder="Deskripsi ringkas unit usaha simpan pinjam..."
              rows={4}
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Keunggulan Kami (Pisahkan dengan baris baru)</label>
            <textarea
              value={benefitsPage}
              onChange={(e) => setBenefitsPage(e.target.value)}
              placeholder="Contoh:&#10;Suku bunga flat kompetitif&#10;Proses pengajuan transparan & kekeluargaan&#10;Bebas biaya administrasi"
              rows={3}
              className="w-full px-4 py-2 bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-xl text-xs text-slate-800 font-medium"
            />
          </div>
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Ketentuan Pinjaman (Pisahkan dengan baris baru)</label>
            <textarea
              value={ratesPage}
              onChange={(e) => setRatesPage(e.target.value)}
              placeholder="Contoh:&#10;Plafon Usaha: Maksimal pinjaman Rp 10.000.000&#10;Tenor Waktu: Fleksibel 3 s/d 12 bulan&#10;Wajib terdaftar anggota aktif BUMDes"
              rows={3}
              className="w-full px-4 py-2 bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-xl text-xs text-slate-800 font-medium"
            />
          </div>
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Persyaratan & Cara Pendaftaran (Pisahkan dengan baris baru)</label>
            <textarea
              value={requirementsPage}
              onChange={(e) => setRequirementsPage(e.target.value)}
              placeholder="Contoh:&#10;Fotokopi KTP suami/istri & Kartu Keluarga&#10;Surat Keterangan Usaha (SKU) dari RT/RW&#10;Pas foto ukuran 3x4 sebanyak 2 lembar"
              rows={3}
              className="w-full px-4 py-2 bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-xl text-xs text-slate-800 font-medium"
            />
          </div>
