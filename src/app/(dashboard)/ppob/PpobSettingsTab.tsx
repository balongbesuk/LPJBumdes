...
export default function PpobSettingsTab() {
  const settings = useSettings() as any
  const [waNumber, setWaNumber] = useState("")
  
  // Page Content Settings
  const [descriptionPage, setDescriptionPage] = useState("")
  const [billingsPage, setBillingsPage] = useState("")
  ...
  useEffect(() => {
    if (settings) {
      setWaNumber(settings.ppob_whatsapp || "")
      setDescriptionPage(settings.ppob_description || "")
      setBillingsPage(settings.ppob_billings || "")
      setServicesPage(settings.ppob_services || "")
      setReasonsPage(settings.ppob_reasons || "")
    }
  }, [settings])
  ...
  const handleSaveSettings = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ppob_whatsapp: waNumber,
        ppob_description: descriptionPage,
        ppob_billings: billingsPage,
        ppob_services: servicesPage,
        ppob_reasons: reasonsPage
      }
      ...
      if (result.success) {
        ...
        setMessage({ type: "success", text: "Pengaturan unit PPOB berhasil disimpan!" })
        setTimeout(() => {
    ...
    <div className="space-y-6 max-w-xl animate-fade-in">
      ...
      <form onSubmit={handleSaveSettings} className="bg-white border border-slate-100 p-6 rounded-3xl shadow-sm space-y-4">
        ...
        <p className="text-slate-400 text-[11px] font-semibold leading-relaxed">
          Masukkan informasi kontak dan isi konten untuk halaman detail Pembayaran Tagihan/PPOB publik.
        </p>
        ...
        <div className="space-y-3 pt-2">
          <div className="space-y-1">
            ...
            <input
              ...
              placeholder="Contoh: 6281234567893"
          ...
          <div className="space-y-1">
            ...
            <textarea
              ...
              placeholder="Deskripsi ringkas unit usaha PPOB..."
              rows={4}
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Tagihan Rumah Tangga (Pisahkan dengan baris baru)</label>
            <textarea
              value={billingsPage}
              onChange={(e) => setBillingsPage(e.target.value)}
              placeholder="Contoh:&#10;Listrik PLN: Token prabayar & pascabayar&#10;PDAM: Air bersih bulanan&#10;BPJS Kesehatan: Iuran jaminan kesehatan"
              rows={3}
              className="w-full px-4 py-2 bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-xl text-xs text-slate-800 font-medium"
            />
          </div>
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Layanan Digital & Pulsa (Pisahkan dengan baris baru)</label>
            <textarea
              value={servicesPage}
              onChange={(e) => setServicesPage(e.target.value)}
              placeholder="Contoh:&#10;Isi ulang pulsa reguler & paket data internet&#10;Top-up saldo e-wallet DANA, OVO, ShopeePay&#10;Pembayaran cicilan motor/mobil"
              rows={3}
              className="w-full px-4 py-2 bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-xl text-xs text-slate-800 font-medium"
            />
          </div>
          ...
          <div className="space-y-1">
            <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wider block">Kenapa Membayar di BUMDes? (Format: Judul: Deskripsi - Pisahkan baris baru)</label>
            <textarea
              value={reasonsPage}
              onChange={(e) => setReasonsPage(e.target.value)}
              placeholder="Contoh:&#10;Aman & Terbuka: Struk bukti fisik resmi berstempel BUMDes&#10;Dekat & Hemat: Loket di Kantor Balai Desa&#10;Membangun Desa: Biaya admin disalurkan kembali sebagai PADes"
              rows={3}
              className="w-full px-4 py-2 bg-slate-50 border border-slate-200 focus:border-emerald-500 focus:bg-white rounded-xl text-xs text-slate-800 font-medium"
            />
          </div>
